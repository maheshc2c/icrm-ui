...
export class AddVisitComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.headerBreadcrumbs = [
            ...
            { label: 'Manage Visits', route: '/salesdirector/planVisit' },
    ...
    loadVisitData() {
        this.SdService.getVisitById(this.visitId).subscribe({
            next: (response: any) => {
    ...
    loadDropdowns() {
        ...
        // const token = this.SdService['auth']?.getToken();
        ...
        this.SdService.getLeads().subscribe({
            next: (leads: any[]) => {
        ...
        this.SdService.getVisitPurposes().subscribe({
            next: (purposes: any[]) => {
    ...
    onSubmit(data: any) {
        ...
        if (this.isEditMode) {
            this.SdService.updateVisit(this.visitId, payload).subscribe({
                next: (response) => {
                    console.log('Visit updated successfully:', response);
                    this.router.navigate(['/salesdirector/planVisit']);
                },
                error: (err) => {
            ...
            this.SdService.createVisit(payload).subscribe({
                next: (response) => {
